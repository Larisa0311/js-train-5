...
function swapMinMax(array) {
  ...
  let [...rest] = array;
  for (const n in rest) {
    if (n == maxIndex) {rest[n] = min}
    else if (n == minIndex) {rest[n] = max}
  }
...
function multiplyByIndex(arr) {
  ...
  for (const item in arr) {
    list[list.length] = item * arr[item];
  }
  return list;
}
...
function replaceNumbers(arr) {
  ...
  for (const item of arr) {
    if (item > 10) {
      arrStr[arrStr.length]  = "Greater than 10";
    } else {
      arrStr[arrStr.length]  = "Less than or equal to 10";
    }
  }
  return arrStr;
}
